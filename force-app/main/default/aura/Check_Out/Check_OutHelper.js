({
    CheckGeoLocation: function(component, event, helper) {
        var action = component.get("c.CheckGeoLocationCheckOut");        
        ...
        action.setCallback(this, function(response) {
            ...
            if (response.getState() === 'SUCCESS' && (response.getReturnValue() != null || response.getReturnValue() != undefined)) {
                ...
                if(geoLocWrap.isAlreadyCaptured == false){
                    ...
                    helper.showToast(component, event, 'Info', $A.get("$Label.c.Check_Out_Already"));                                                 
                }
            } else {
                component.set("v.isButtonDisabled", false);
                helper.showToast(component, event, 'Error' ,$A.get("$Label.c.Check_Out_Error")+' : ' + response.responseText);                
            }
        });
    ...
    updateGeoLocation: function(component, event, helper, geoObj) {
        var action = component.get("c.UpdateCheckInLocationCheckOut");
        var recId = component.get("v.recordId");
        ...
        action.setParams({
            ...
            "Accompanied" : selectedOption,
            "AccompaniedWith" : selectedUserId
        });
