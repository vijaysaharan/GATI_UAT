({
    itemSelected : function(component, event, helper) {
        ...
        if(SelIndex){
            ...
            if(selItem.val){
                console.log('=====>child111111=====>'+selItem.val);
                var compEvent = $A.get("e.c:DynamicReUseableLookUp_Event"); 
                //var compEvent = component.getEvent("TempId");
                compEvent.setParams({"templateId" : selItem.val});
                console.log('=====>child222222=====>'+selItem.val);
                compEvent.fire();
                console.log('=====>child333333=====>'+selItem.val);
    ...
    serverCall : function(component, event, helper) {  
        ...
        if (event.keyCode == 27 || !searchText.trim()) { 
            ...
            // && /\s+$/.test(searchText) // removed from above if from not giving space
    ...
    clearSelection: function(component, event, helper){
        ...
        var compEvent = $A.get("e.c:DynamicReUseableLookUp_Event"); 
        //var compEvent = component.getEvent("TempId");
        compEvent.setParams({"templateId" : ""});
