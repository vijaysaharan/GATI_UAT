({
    helperMethod : function() {
        if(resultsToast)
        {
            if (result.state === "SUCCESS") {
                console.log('Checking State',result.state)
                resultsToast.setParams({
                    "title": "Saved",
                    "message": "The Feedback was submitted."
                });
                resultsToast.fire();                
                ...
                resultsToast.setParams({
                    "title": "Error",
                    "message": "There was an error submitting the record: " + JSON.stringify(result.error)
                });
                resultsToast.fire();
            } else {
    ...
    saveRecord : function(component, event, helper) {
        var tempRec=component.find("CustomerFeedback")
        console.log('tempRec',component.find("CustomerFeedback"))
        tempRec.saveRecord($A.getCallback(function(result) {
            console.log('State:',result.state);
            var resultsToast = $A.get("e.force:showToast");
            if (result.state === "SUCCESS") {
                console.log('Inside IF State:',result.state)
