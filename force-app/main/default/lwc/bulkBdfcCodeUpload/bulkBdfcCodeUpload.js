...
import BDFS_UPLOAD from '@salesforce/resourceUrl/bdfsCodes';
import { loadScript } from 'lightning/platformResourceLoader';
import CsvDataHandler from '@salesforce/apex/BulkBdfsCodesUpload.CsvDataHandler';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';

export default class BulkBdfcCodeUpload extends NavigationMixin(LightningElement) {
    ...
    downloadDummy(){
        ...
        window.open(baseUrl + BDFS_UPLOAD);
    }
    cancel(){
        this.rowsTotal = [];
        this.dataForResult = [];
    ...
    handleSave(event){
        this.isloading = true;
        console.log('rowsTotal',JSON.stringify(this.rowsTotal,null,2));
        CsvDataHandler({csvDataList : JSON.stringify(this.rowsTotal)}).then(res => {
            if(res){
                ...
                let data = ['CustomerCode','BDFSCode','Message'];
    ...
    handleDownload(event){
        ...
        downloadElement.download = 'BDFSCode'+currentName+'.csv';
