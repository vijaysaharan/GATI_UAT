...
export default class AddCall extends LightningElement {
    @track parent;
	//@track OJTRecordId;
	@api factorform;
	@api usrid;
    ...
    handleSubmits(event) {
		...
		fields.OJT_Feedback_Form__c = this.parent;
		...
		this.template.querySelectorAll("lightning-input-field").forEach((element) => {
			console.log(`Element is ${element.name} the value is ${element.value} row is ${element.dataset.id}`);
			 if (arr.includes(element.value) && element.value!=null) {
			   ...
			   isVal = false;
         alert(`Duplicate Call found for ${element.name} at row ${parseInt(element.dataset.id) + 1}`);
         return isVal;
